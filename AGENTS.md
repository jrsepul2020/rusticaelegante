...
- Este demo usa el teléfono/WhatsApp `611 82 94 14`; el brief indica `955 498 119` y especifica llamadas telefónicas, sin reservas online.
- Este demo es oscuro y usa Inter/Oswald; el brief define una base crema clara y una única tipografía Poppins.
...
- El directorio no está inicializado como repositorio Git.
...
- `script.js`: menú móvil, movimiento/parallax del hero y carrusel con autoplay.
...
- `carta-rustica-napoletana/carta.js`: menú móvil y detección de categoría visible.
...
- `internas.js`: comportamiento compartido de sus menús móviles.
...
El sitio tiene cinco entrypoints:

1. `/index.html`: portada; carga `styles.css` y `script.js`.
2. `/carta-rustica-napoletana/index.html`: carta; reutiliza `styles.css` y añade `carta.css` y `carta.js`.
3. `/eventos-rustica-napoletana/index.html`: celebraciones; carga `styles.css`, `internas.css` e `internas.js`.
...
No existe estado persistente ni intercambio de datos. Las conversiones salen del sitio mediante:

- WhatsApp: `https://wa.me/34611829414`.
...
1. Header responsive con marca, navegación, reserva y teléfono.
2. Hero con propuesta de valor, CTAs, reconocimientos y pizza animada.
3. Categorías visuales de la carta.
4. Carrusel de cuatro pizzas.
5. Selección resumida de la carta real.
6. Avance de “Sobre nosotros” con enlace a `/nosotros/`.
7. CTA final de ubicación/reserva.
...
4. Carta editorial completa con platos, descripciones, etiquetas y precios.
5. Aviso final sobre precios y alérgenos.
...
- El botón móvil alterna la clase `menu-open` en `.site-header` y sincroniza `aria-expanded`.
...
- `carta.js` marca la categoría visible y controla el menú móvil de la carta.
- `internas.js` controla el menú responsive compartido por Eventos, Novedades y Nosotros.

## Sistema visual

Dirección: editorial, oscura, gastronómica y de alto contraste.
...
- No cambiar el teléfono o WhatsApp en un único lugar: aparecen varias veces en `index.html`.
...
- Teléfono y disponibilidad real de WhatsApp; el brief relacionado contiene un número distinto.
- Dirección física y, si se añade, enlace de mapas.
- Horarios y canales de reserva.
...
- No hay footer, dirección completa, horarios, mapa, política de privacidad, cookies ni aviso legal.
- La reserva abre WhatsApp sin mensaje precargado y no existe confirmación dentro del sitio.
...
3. Probar enlaces de WhatsApp y teléfono sin completar acciones reales.
